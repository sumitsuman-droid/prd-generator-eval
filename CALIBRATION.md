...
|---------|--------------|-------------|------------|--------------|
| v1 | 8.86 | 3.00 | 5.88 | Initial run, original rubric |
| v2 | 8.86 | 3.00 | 5.86 | Rubric tightened; per-dim analysis flagged user_centricity gap |
| v3 | 9.04 | 2.48 | 6.56 | Generator fix: persona threading constraint added |
...
**Rubric tightening hurt weak scores more than it helped strong ones.** The v1 → v2 change was a small rubric edit, but looking at the per-dimension breakdown, it compressed completeness and user_centricity separation the most (3.8 and 4.3 respectively). Strong PRDs barely moved; weak PRDs stopped getting partial credit for having the right section headings with hollow content. That asymmetry is what made the user_centricity problem visible: weak PRDs were scoring relatively high on that dimension compared to everything else, which flagged that something was off.
